...
class Utility:
    def saveUser(self, conn, cursor, request):
        try:
            ...
            cursor.execute("INSERT INTO users(uid,fname,lname, email,phone, team, redhatid, role) VALUES(%s,%s,%s,%s,%d,%s,%s,%s)",(uid,fname,lname, email,int(phone), team, redhatid, role))
            conn.commit()
            print("yay!")
            return 200
